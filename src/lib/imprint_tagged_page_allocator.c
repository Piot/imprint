...
void imprintTaggedPageAllocatorInit(ImprintTaggedPageAllocator *self,
...
void imprintTaggedPageAllocatorDestroy(ImprintTaggedPageAllocator *self) {
  for (size_t i = 0; i < self->entryCapacity; ++i) {
    ...
    if (entry->tag) {
      CLOG_ERROR("tag %016lX with pages %016X was not cleared", entry->tag,
                 entry->pageIds);
...
void imprintTaggedPageAllocatorAlloc(ImprintTaggedPageAllocator *self,
                                     uint64_t tag, size_t pageCount,
...
void imprintTaggedPageAllocatorFree(ImprintTaggedPageAllocator *self,
                                    uint64_t tag) {
  for (size_t i = 0; i < self->entryCapacity; ++i) {
    ImprintTaggedPageEntry *entry = &self->entries[i];
    if (entry->tag == tag) {
