...
void imprintPageAllocatorInit(ImprintPageAllocator *self, size_t pageCount) {
  ...
  CLOG_VERBOSE("=== Allocated all page memory %zu (%zu count)",
               self->pageSizeInOctets, self->pageCount);
...
void imprintPageAllocatorAlloc(ImprintPageAllocator *self, size_t pageCount,
  ...
  for (size_t i = 0; i < 64 - pageCount; ++i) {
    ...
    if (usedAndRequestMask == requestMask) {
      ...
      CLOG_VERBOSE("pages %016X allocated (%zu page count)", requestMask,
                   pageCount)
      return;
    }
  ...
  CLOG_ERROR("page allocator: out of memory pageCount %zu freeMask %016X",
             pageCount, self->freePages);
}

void imprintPageAllocatorFree(ImprintPageAllocator *self,
  ...
  tc_memset_octets(self->basePointerForPages + start * self->pageSizeInOctets,
                   0xbd, pageCount * self->pageSizeInOctets);

#endif
}

void imprintPageAllocatorFreeSeparate(ImprintPageAllocator *self,
                                      ImprintPageIdList pageIds) {
  self->freePages |= pageIds;
